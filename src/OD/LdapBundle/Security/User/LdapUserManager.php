...
class LdapUserManager implements LdapUserManagerInterface
{
     ...
     */
    public function __construct(Ldap $ldap, $userBaseDn, $userFilter, $usernameAttribute,
                                $roleBaseDn, $roleFilter, $roleNameAttribute, $roleUserAttribute)
    {
        $this->ldap = $ldap;
    ...
    public function getUsernames()
    {
        return $this->resolveEntriesToAttributes($this->ldap->searchEntries(
                    $this->userFilter, $this->userBaseDn, Ldap::SEARCH_SCOPE_SUB,
    ...
    {
        return $this->resolveEntriesToAttributes($this->ldap->searchEntries(
                    $this->roleFilter, $this->roleBaseDn, Ldap::SEARCH_SCOPE_SUB,
    ...
    public function getRolesForUsername($username)
    {
        return $this->resolveEntriesToAttributes($this->ldap->searchEntries(
                    ...
                    $this->roleBaseDn, Ldap::SEARCH_SCOPE_SUB, array($this->roleNameAttribute)
                ), $this->roleNameAttribute);
    }
     ...
     * values.
     *
     * @param array  $entries       the search entry
